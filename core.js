/**
 * Google Tasks MCP — 共通コア
 * Zero dependencies — uses only Node.js built-ins.
 *
 * 認証・Google Tasks API 呼び出し・ツール定義・ツール実行を集約する。
 * stdio エントリ (index.js) と HTTP/SSE エントリ (server.js) の両方から require される。
 */
"use strict";
...
async function getAccessToken() {
  ...
  if (!data.access_token) {
    // 生のレスポンスはサーバーログのみに残し、クライアントには汎用メッセージを返す
...
async function callTool(name, args) {
  switch (name) {
    ...
    case "create_task":
      return createTask(args?.taskListId, args.title, args?.notes, args?.due, args?.parent);
    case "update_task": {
      const patch = {};
...
module.exports = { TOOLS, callTool, getAccessToken };
